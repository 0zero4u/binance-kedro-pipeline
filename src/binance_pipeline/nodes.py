...
def sample_features_to_grid(df: pd.DataFrame, rule: str = '25ms') -> pd.DataFrame:
    ...
    # --- ADDED LOGGING ---
    log.info(f"  - Input shape to sampling node: {df.shape}")
    ...
    # --- ADDED LOGGING ---
    log.info(f"  - Shape after resampling and ffill: {sampled_df.shape}")
    
    # 3. Drop initial NaNs generated by the EWMA warm-up period
    # --- FIX: Changed overly aggressive dropna to a targeted one ---
    log.info("  - Applying targeted dropna to handle EWMA warm-up period...")
    sampled_df.dropna(subset=['mid_price_ewma_5s'], inplace=True)
    # --- ADDED LOGGING ---
    log.info(f"  - Shape after targeted dropna: {sampled_df.shape}")
...
def generate_bar_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- ADDED LOGGING ---
    log.info(f"  - Input shape to bar features node: {df.shape}")
    df = df.copy()
    ...
    # --- ADDED LOGGING ---
    log.info(f"  - Shape before final dropna in bar features: {df.shape}")
